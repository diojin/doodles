...
sub start_gf_agent{
	...
	print "about to start $_[0] agent.\n";
	chdir $gf_settings{$basemodule}{"agent".$writerflag};
	dos_command_no_output(sprintf( "agent start -dir=%s	property-file=%s", 
				$gf_settings{$basemodule}{"agent".$writerflag},
				$proj_settings{$basemodule}{"basedir"}."/".$proj_settings{$basemodule}{"agent".$writerflag} ));
	$status = gemfire_agent_status($gf_settings{$basemodule}{"agent".$writerflag});
	while ( $status ne "running" ){
		if ( $status ne "starting" ){
			print "### Failed to start the $_[0] agent ###\n";
			return 0;
		}
		sleep 10;
		$status = gemfire_agent_status($gf_settings{$basemodule}{"agent".$writerflag});
	}			
	print "$_[0] agent is up and running.\n";				
}
